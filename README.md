...
#### Keep (좋은 결과, 계속 유지해나가야 할 것)
- 팀원들 모두 열정적으로 임하며 다양한 아이디어를 제시하며 적극적으로 회의에 참여하여 다양한 후보 중에 주제를 선정하는 과정이 원활
- 주제 선정 뿐만 아니라 설계부터 개발완료 까지 모든 과정에서 의견에 뒷받침 되는 근거를 제시하고 상대를 배려하는 마음이 있어 원활한 의사소통으로 모든 회의가 무리없이 진행
- 계획했던 기능의 90%이상 구현에 성공
- 매일 작업 시작 전 스탠딩 회의, Action Item회의, Risk Mitigation작업을 진행
- 매번 회의 내용과 중간중간 논의가 필요한 점 문제가 발생한 점에 대한 꼼꼼한 기록
- 멘토님께서 현업에 대한 정보를 많이 주셨다. 의견 조율 방식, 프로젝트 진행 방식, 프로젝트 계획 방식, CI/CD등

#### Problem (아쉬운 결과, 앞으로 개선되어야 할 것)
- 주제선정에 시간소요를 방지하기 위해 미리 준비하였음에도 모두가 거의 처음 프로젝트를 진행했기 때문에 진행과정이 예측되지 않아 주제를 선정하기 쉽지 않아 시간소요가 예상보다도 많이 되었다.
- 기능 구현을 하던 중 막히게 되면 해결하고 넘어가고 싶은 마음에 안되는 일을 너무 붙잡고있다보니 효율적인 진행이 어려웠다.
- 기한 내에 기능 구현을 목표로 두다보니 작성한 모든 코드를 세세하게 이해하지 못하고 넘어간 부분
- 이슈 및 코드 정리를 세세하게 기록하지 못한 부분

#### Try (문제 파악, 이를 해결하기 위한 구체적인 개선방안)
-  모든 이슈와 코드에 대한 해석과 다른 대안을 기록해 두었다면 같은 실수를 미연에 방지 하고 시간이 지나서도 값어치 있는 결과물이 되었을 것이다.
- 욕심을 버리고 분배된 시간을 잘 지켰더라면 효율적인 진행이 가능했을 것이다.
- 테스트 코드를 작성
- 코드를 보기좋게 정리하여 기록

 첫 프로젝트를 마무리하고 돌아보니 좋은 사람들과 열심히 달려온 약 세달간의 여정에서 배운것도 많고 아쉬움도 많이 남았지만 그럼에도 뿌듯함이 앞서는 값진 결과를 얻었다.<br>
 회사생활에서 데이터 분석을 접하게 되며 처음으로 흥미로운 분야가 생겼다. 데이터 분석 스킬 향상을 위해 부트캠프를 시작하였고 데이터 분석 분만 아니라 Front-End, Back-End를 접하게 되며 시야가 넓어졌다. <br>
 프로젝트를 진행하며 Front-End, Back-End를 담당하게 되었다. 처음 접하는 스킬과 툴을 사용하며 스스로의 부족함도 많이 느꼈지만 팀원들과의 원활한 커뮤니케이션과 멘토님과 교수님의 지도아래 기한안에 만족스로운 결과물을 얻을 수 있었다. <br>
앞으로도 더 전문성을 갖추기 위해 더 많은 노력을 하며 더욱 성장할 것이다. 
